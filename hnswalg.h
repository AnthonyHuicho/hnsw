...
namespace hnswlib
{
    ...
    public:
        ...
        size_t ef_{0};
        //Parametros ideales indicados por el paper ubicados en la tabla 3
        double parametroUmbralGap{0.010};
        double parametroUmbralRatio{1.008};
        size_t parametroReranK{28};
        ...
        void setAqrParams(float tau_gap, float tau_ratio, size_t n_rerank, bool enabled = true)
{
    parametroUmbralGap   = tau_gap;
    parametroUmbralRatio = tau_ratio;
    parametroReranK      = n_rerank;
}
...
//                    if (candidate_id == 0) continue;
        ...
        //funcion earlytermination sacado del paper y implementado para usar con el knn
        bool early_termination_Paper (const std::vector<std::pair<dist_t,tableint>>& candidatos, size_t k)const {
            if (candidatos.size()<=k){
                return false;
            }
            else{
                float epsilon= 1e-6f;
                dist_t d_ka= candidatos[k-1].first;
                dist_t d_ka1= candidatos[k].first;
                float gapEarly = (d_ka1 -d_ka)/(d_ka+epsilon);
                float ratioEarly = d_ka1/(d_ka+epsilon);
                return ((gapEarly>parametroUmbralGap) || (ratioEarly> parametroUmbralRatio));
            }
        }
        ...
        searchKnn(const void *query_data, size_t k, BaseFilterFunctor *isIdAllowed = nullptr) const
        {
            ...
            //copio el topcandidates para no malograr el vector original\

              //algoritmo del AQR, algoritmo 2 implementacion
            if( top_candidates.size() > k ){

                std::priority_queue<std::pair<dist_t, tableint>, std::vector<std::pair<dist_t, tableint>>, CompareByFirst> copiaTopCandidates = top_candidates;
          
                std::vector<std::pair<dist_t, tableint>> candidatosMejores;
                while(!copiaTopCandidates.empty()){
                    candidatosMejores.push_back(copiaTopCandidates.top());
                    copiaTopCandidates.pop();
                }
                std::reverse(candidatosMejores.begin(), candidatosMejores.end());
                if(early_termination_Paper(candidatosMejores, k)==false){
                    size_t rerank= std::min(parametroReranK, candidatosMejores.size());
                    for (int i=0;i<rerank;i++){
                        candidatosMejores[i].first=fstdistfunc_(query_data, getDataByInternalId(candidatosMejores[i].second), dist_func_param_ );
                    }
                    std::sort(candidatosMejores.begin(),candidatosMejores.begin()+ rerank, [](const std::pair<dist_t, tableint>& a,const std::pair<dist_t, tableint>& b)
                    { return a.first < b.first; });
                }
                size_t take = std::min(k, candidatosMejores.size());
                for (size_t i = 0; i < take; i++)
                     result.push({candidatosMejores[i].first,
                     getExternalLabel(candidatosMejores[i].second)});
                    return result; } 
            return result;
        }
